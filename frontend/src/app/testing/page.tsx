...
const MODEL_TYPE_META: Record<string, ModelTypeMeta> = {
  ...
  infinity_parser: {
    label: "Infinity-Parser2-Flash",
    short: "Infinity",
    description: "Fast document parsing with layout, tables, charts and Markdown output.",
    icon: Infinity,
    bg: "from-sky-500/15 via-blue-500/8 to-sky-500/5",
    iconBg: "bg-gradient-to-br from-sky-500 to-blue-500 shadow-sky-500/30",
    ...
    api
      ...
      .then((data) => {
        const filteredModels = data.models.filter((m) =>
          SUPPORTED_TESTING_MODEL_TYPES.has(m.type)
        );
          ...
          <FadeIn delay={0.05}>
            <div className="space-y-6 rounded-3xl border border-border/60 bg-card shadow-md">
              ...
              <div className="space-y-6 px-6 pb-6">
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        className="flex h-11 w-full rounded-xl border-2 border-input bg-background px-4 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-500/20"
                      >
                    ...
                    <div className="space-y-2">
                      <Label
                        htmlFor="infinity-instruction"
                        className="text-xs font-bold uppercase tracking-wider text-muted-foreground"
                      >
                        Custom instruction (optional)
                      </Label>
                      <Textarea
                        id="infinity-instruction"
                        value={customInstruction}
                        onChange={(e) => setCustomInstruction(e.target.value)}
                        rows={3}
                        placeholder="Override the default prompt for this output format…"
                        className="resize-y rounded-xl border-2 focus-visible:ring-sky-500/20"
                      />
                    </div>
                  </>
                )}
                ...
                {supportsThinking && (
                  <label className="flex cursor-pointer items-center gap-3 rounded-xl border-2 border-border bg-muted/20 px-4 py-3">
                    <input
                      type="checkbox"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                      ...
                      {result.result.output_format && (
                        <Tile
                          label="Format"
                          value={formatOutputFormatLabel(result.result.output_format)}
                          accent="cyan"
                        />
                      )}
